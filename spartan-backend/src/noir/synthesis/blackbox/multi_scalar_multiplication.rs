...
pub fn handle_msm<CS: ConstraintSystem<Scalar>>(
    ...
    let point = AllocatedPoint {
        ...
        is_infinity: AllocatedNum::alloc(&mut *cs, || Ok(Scalar::zero()))?,
    };

    let scalar = allocate_or_get(
        allocation_store,
        &mut *cs,
        // high part of the scalar. We don't need the low part for T-256-P-256 since the field
    ...
    enforce_equal(cs.namespace(|| "MSM x is correct"),  &result.x, &get_witness_assignment(allocation_store, &outputs.0)?);
    enforce_equal(cs.namespace(|| "MSM y is correct"),  &result.y, &get_witness_assignment(allocation_store, &outputs.1)?);
