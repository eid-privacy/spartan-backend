...
pub fn instantiate_circuit_with_name(name: &str) -> CircuitParameters {
    ...
    instantiate_circuit_with_settings(name, circuit_settings, true, true)
}
...
pub fn instantiate_prover_circuit_with_name(name: &str) -> CircuitParameters {
    let circuit_settings = CircuitSettings::new(name);
    instantiate_circuit_with_settings(name, circuit_settings, true, false)
}
...
// Instantiates a circuit for verification only, by name
pub fn instantiate_verifier_circuit_with_name(name: &str) -> CircuitParameters {
    let circuit_settings = CircuitSettings::new(name);
    instantiate_circuit_with_settings(name, circuit_settings, false, true)
}

// Instantiates a circuit for verification only
pub fn instantiate_verifier_circuit_from_dir(dir: &Path) -> CircuitParameters {
    let circuit_settings = CircuitSettings::from_directory(dir);
    let name = dir
        .file_name()
        .expect("directory path must have a final component")
...
fn instantiate_circuit_with_settings(
    ...
    let field_prover_input = if require_prover_inputs {
